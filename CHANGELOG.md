...
- feat: octopus verdict emission — the execution phase now emits the octopus-analyse-bugs verdict schema (`verdict` ∈ real bug | noise | duplicate | closed-fixed-in-prod | not-a-defect | track, plus `understanding`, `fix_certainty`, `root_cause`, `recommended_fix`, `file:line`, `disqualifiers_fired`, `live_event_count`), replacing the triage-style `confidence`/`reason`/`last_seen`/`sentry_status` keys. `pkg/verdict` struct + validation aligned (real-bug verdicts now require `file:line`, `root_cause`, `recommended_fix`, and High/Medium/Low U/F); downstream trigger keys on `understanding: High` AND `fix_certainty: High`.
