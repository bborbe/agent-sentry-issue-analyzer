...
- Deleting the two deep manifests orphans the deep analyzer's dedicated GitHub App (`APP_ID 4710983` / `INSTALLATION_ID 156399284`, provenance `CHANGELOG.md:74`). Verified safe: the live `sentry-analyzer-agent` CR carries its own different App (`APP_ID 4710998` / `INSTALLATION_ID 156399409`) plus `Bash(scripts/repo-clone.sh:*)` in `ALLOWED_TOOLS`, so private-repo cloning is unaffected. Retiring App 4710983 and its TeamVault PEM is out of scope and tracked separately.
