...
var _ = Describe("ReassignExecutionStep", func() {
	...
	// instruction as a word - observed as `unknown` on 2026-09-12 and
	// `unavailable` on 2026-09-13, because it echoes the prompt's own adjective.
	//
	// Both production blocks run through the whole step, not only through Parse.
	// The two failure modes are distinct: a non-numeric token fails the int
	// unmarshal, while a non-date token unmarshals fine into the string field
	// and only fails later, in applyDisqualifiers' date parse. A fix to the int
	// alone leaves the Job still exiting 1 - so covering one block would miss
	// the half that let this crash survive its first "fix".
	DescribeTable(
		"does not error when the live-state fields carry a not-evaluable token",
		func(block string) {
		...
		Entry(
			"the 2026-09-12 block (`unknown` on the int field)",
		...
		Entry(
			"the 2026-09-13 block (`unavailable` on the int field)",
